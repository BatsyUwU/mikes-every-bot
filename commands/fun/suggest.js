...
module.exports = {//Command Configuration
    ...
    run: async(bot, message, args)=>{//Running the command(Writing our code so it could import it)
        var channel = message.guild.channels.cache.get(db.get(`suggestchan_${message.guild.id}`))//Gets the channel
        if(channel==null){
            return message.channel.send(`There isn\'t a suggestion channel setup! Set one up by using the ${bot.guildPrefixes.get(message.guild.id)}setsuggestions <mentioned channel,id> command!`)
        }
        ...
        m.react("717980106404134943")
        m.react("725122241230143529")
